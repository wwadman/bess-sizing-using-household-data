...
from src.batteries.bess_type import BessType

bess_candidates = []
bess_candidates += BessType(
    name='Zendure SolarFlow 2400 AC+',
    extension_name="AB3000L",
    capacities=[2.4, 5.3, 8.2, 11.0],
    charging_rates=[2.4] * 4,
    rtes=[.8815] * 4,  # Could not find info on rte of additional modules, so assuming equal
    prices=[1089, 1818, 2547, 3276]
...
bess_candidates += BessType(
    ...
    capacities=[7 * (i + 1) for i in range(2)],
    charging_rates=[3.5] * 2,
    rtes=[.835] * 2,  # Could not find info on rte of additional modules, so assuming equal
    prices=[2499 + 1699 * i for i in range(2)]
).get_all_possible_besses_of_this_type()
